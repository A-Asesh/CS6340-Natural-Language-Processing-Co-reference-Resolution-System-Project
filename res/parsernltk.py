...
def main():
	...
	for file in inputfiles_list:
		...
		#print len(coref_ids)
		#print coref_ids
		#np = np_extraction.main(inFileread)
		#print np
		#inputfileforcluster(file,coref_ids,np)
		#inFile2 = open(file, 'r')
		#print inFile2.read()

		#coref_ids_final = parsexml(file)
		#print(coref_ids_final)
		Final_Result = Feature_Copy.main(coref_ids,ma_file[6:],fe_file[6:])
		Outputfile	= corefoutput.main(Final_Result)
		inFile.close()
		fwrite = open('/home/asesh/NLP_Proj_Final/res/responses/'+filename+'.response', 'w')
		fwrite.write(Outputfile)
		fwrite.close()
	responsefile = open('/home/asesh/NLP_Proj_Final/res/responselist.txt', 'w')
	for file in inputfiles_list:
		file_temp = file.split('/')[-1]
		filename = file_temp.split('.')[0]
		#print filename
